...
LightVisibilityTester::LightVisibilityTester(const Ray& ray, IntersectInfo& info, const float& epsilon, const std::vector<std::unique_ptr<SceneObject>>& objects, const std::vector<std::unique_ptr<Light>>& lights)
...
{
	bVisible = PathTracing::TraceRay(ray, info, epsilon, objects, lights) && info.shape == light.GetShape();
}
